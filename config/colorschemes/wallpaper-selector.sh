...
COLORSCHEMES_DIR="$HOME/nixos/config/colorschemes"
...
selected=$(echo -en "$menu_options" | rofi -dmenu -i -p "Select Wallpaper" -show-icons -theme ~/nixos/config/rofi/launchers/type-1/style-5.rasi)
...
swww img "$selected_path" --transition-type wipe --transition-fps 165 --transition-step 255
...
ln -sf "$selected_path" ~/nixos/config/hypr/hyprlock/wallpaper
