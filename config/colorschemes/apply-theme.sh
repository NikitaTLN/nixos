...
THEME="$1"
THEME_DIR="$HOME/nixos/config/colorschemes/$THEME"
WALLPAPER_STATE="$HOME/nixos/config/colorschemes/.wallpaper-state"
...
# Track current theme
CURRENT_THEME_FILE="$HOME/nixos/config/colorschemes/.current-theme"
...
cp "$THEME_DIR/hypr/colors.conf" "$HOME/nixos/config/hypr/colors/colors.conf" > /dev/null 2>&1
echo ""

# Waybar style
echo -e "${CYAN}-> Applying Waybar CSS...${NC}"
cp "$THEME_DIR/waybar/colors.css" "$HOME/nixos/config/waybar/colors/colors.css" > /dev/null 2>&1
echo -e "${CYAN}-> Restarting Waybar...${NC}"
pkill waybar > /dev/null 2>&1 && ~/nixos/config/waybar/scripts/launch.sh > /dev/null 2>&1 & disown
echo ""
...
GTK4_DST="$HOME/nixos/config/gtk-4.0"
...
case "$THEME" in
    everforest-dark|gruvbox-dark|catppuccin|tokyo-night|kanagawa|nord-darker|noir|e-ink|nightfox|rose-pine)
        cp "$THEME_DIR/kitty/colors.conf" "$HOME/nixos/config/kitty/colors/colors.conf" > /dev/null 2>&1
        ;;
    *)
...
cp "$THEME_DIR/swaync/colors.css" "$HOME/nixos/config/swaync/colors/colors.css" > /dev/null 2>&1
pkill swaync > /dev/null 2>&1 && swaync > /dev/null 2>&1 & disown
echo ""

# wlogout theme
echo -e "${CYAN}-> Applying wlogout theme...${NC}"
cp "$THEME_DIR/wlogout/colors.css" "$HOME/nixos/config/wlogout/styles/colors.css" > /dev/null 2>&1
echo ""

# Rofi theme
echo -e "${CYAN}-> Applying Rofi theme...${NC}"
cp "$THEME_DIR/rofi/colors.rasi" "$HOME/nixos/config/rofi/theme.rasi" > /dev/null 2>&1
echo ""

# NvChad theme
echo -e "${CYAN}-> Applying Neovim theme...${NC}"
cp "$THEME_DIR/nvim/lua/colors.lua" "$HOME/nixos/config/nvim/lua/colors.lua" > /dev/null 2>&1
