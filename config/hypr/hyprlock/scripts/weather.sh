...
CITY=$(grep -oP '^\s*\$CITY\s*=\s*\K.+' ~/nixos/config/hypr/hyprlock/hyprlock.conf | xargs)
COUNTRY=$(grep -oP '^\s*\$COUNTRY\s*=\s*\K.+' ~/nixos/config/hypr/hyprlock/hyprlock.conf | xargs)
